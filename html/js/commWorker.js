...
var APP = {
  Controller: {
    init: function (options) {
      ...
      $(options.deleteMailBtn).unbind("click").bind("click", this.handleDelete);
      $(options.form).submit(this.handleSubmit);
    },
    setHeaderEmail: function (email) {
      $(".currEmail").text(" " + this.mail + "@mailblanao.space");
    },
    ...
    getNewEmail: function () {
      ...
      if (email) {
        email = email.split("@")[0];
        $.cookie("squlear", email, {
          expires: 2,
          path: "/",
          domain: "mailblanao.space",
        });
    ...
    getListItem: function (mail) {
      ...
      tmp = tmp.replace("{{$from}}", APP.Utils.formatFrom(mail.from.value[0]));
    ...
    getMailItem: function (mail) {
      ...
      tmp = tmp.replace("{{$to}}", mail.to.value[0].address);
      tmp = tmp.replace("{{$subject}}", mail.subject);
      tmp = tmp.replace("{{$emailId}}", mail._id);
      ...
      tmp = tmp.replace("{{$body}}", mail.html || mail.textAsHtml);
      return tmp;
    },
    getMails: function (mailAddr) {
      var URL = this.inboxUrl + mailAddr + "@mailblanao.space";
      $.ajax({
        type: "GET",
        url: URL,
        dataType: "json",
  ...
  WS: {
    init: function () {
      ...
      this.socket.on("add-consumer", function () {
        ...
        APP.WS.socket.emit(
          "add-consumer",
          APP.Controller.mail + "@mailblanao.space"
        );
        console.log(
          "sent add-consumer " + APP.Controller.mail + "@mailblanao.space"
        );
      });
  ...
  Utils: {
    ...
    formatFrom: function (from) {
      var name =
          '<span class="addressName">' + from.address.split("@")[0] + "</span>",
        host =
          '<span class="addressHost">' +
          "@" +
    ...
    formatDate: function (dateObj) {
      var date =
          '<span class="recDate">' + dateObj.toLocaleDateString() + "</span>",
        time =
          '<span class="recTime">, ' + dateObj.toLocaleTimeString() + "</span>";
